...
const deletePhoto = async (req, res) => {
	...
	try {
		...
		if (!photo.userId.equals(reqUser._id)) {
			res.status(422).json({
				error: ["This ID does not belongs to you. "],
			});
		}
...
const updatePhoto = async (req, res) => {
	...
	if (!photo.userId.equals(reqUser._id)) {
		res.status(422).json({
			errors: ["This ID does not belongs to you."],
		});
		return;
	}
	...
	await photo.save();
	res.status(200).json({ photo, message: "Updated photo successfully!" });
};
